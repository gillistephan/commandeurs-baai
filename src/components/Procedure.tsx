...
import { Box, Circle, Flex, Heading, Icon, Image, Stack, StackProps, Text } from "@chakra-ui/react";
  ...
  const items = React.useMemo(
    () =>
      React.Children.toArray(children)
        ...
        .map((item, index, array) => (index + 1 === array.length ? React.cloneElement(item, { isLastItem: true }) : item)),
    [children]
  );
  return (
  ...
  return (
    <Stack as="li" direction="row" spacing="4" {...stackProps}>
      <Flex direction="column" alignItems="center" aria-hidden="true">
        <Circle bg="yellow.300" size="16" borderWidth="4px" borderColor="white" color="white">
          {icon}
        </Circle>
    ...
    <Box as="section" py={{ base: "12", md: "20" }} maxW="7xl" mx="auto">
      ...
      >
        <Box maxW="3xl" mx="auto" p={{ base: "4", md: "8" }}>
          <Heading color="gray.700" size="2xl" fontWeight="extrabold" lineHeight="normal" mb="12" letterSpacing="tight">
            Show us your support by
          </Heading>
          <List spacing="12">
            <ListItem title="Donate to our Foundations" icon={<Icon as={AiOutlineCreditCard} boxSize="7" />}>
              <div>
            ...
            <ListItem title="Share the beauty of Aruba's Nature" icon={<Icon as={AiOutlineShareAlt} boxSize="7" />}>
              <Text color="gray.700" fontSize="lg">
                We wish to call on all who feel connected to the island nature to share photos, videos and other images on our facebook
                page, which show the beauty in Aruba's nature.
              </Text>
            </ListItem>
          </List>
        </Box>
        <Box pos="relative" w={{ base: "full" }} h={{ base: "auto", lg: "660px" }}>
          <Image w="full" pos="relative" zIndex="1" h={{ lg: "100%" }} objectFit="contain" src="/the_maskot.jpg" />
          <Box pos="absolute" w="100%" h="100%" top="-4" left="-4" bg="gray.50" />
        </Box>
      </Stack>
    </Box>
  );
